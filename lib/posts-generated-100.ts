...
function immediateBody(topic: Topic, index: number) {
  ...
  const deepDive = [
    `<h2>실전 일정 예시: 첫차부터 하산까지</h2>
<p>예를 들어 오전 6시 40분에 북한산우이역에 도착한다고 가정하면, 들머리 정비와 화장실 이용까지 포함해 실제 산행 시작은 오전 7시 전후가 됩니다. 이때 능선 전체를 욕심내기보다 오전 10시 30분에 첫 번째 판단 지점을 잡는 것이 좋습니다. 컨디션이 좋으면 다음 능선으로 이어가고, 발목이나 무릎 피로가 빠르게 오면 가까운 하산로로 내려옵니다.</p>
<p>수락산-불암산 연결 산행도 같은 방식으로 봐야 합니다. 수락산 정상에 늦게 도착했다면 불암산까지 무리하게 잇지 말고 당고개역 방향으로 내려오는 편이 낫습니다. 능선 산행에서 좋은 계획은 가장 긴 코스를 완주하는 계획이 아니라, 중간에 줄여도 하루가 망가지지 않는 계획입니다.</p>
<h2>이 글의 추천 독자와 제외 대상</h2>
<p>이 코스 판단법은 차 없이 산에 가고, 하산 후에도 전철이나 버스로 귀가해야 하는 사람에게 맞습니다. 반대로 암릉 경험이 거의 없거나, 해가 진 뒤 산길을 걸어본 적이 없다면 긴 능선 연결보다 짧은 원점회귀부터 시작하는 편이 좋습니다. 검색 상위 글의 “추천 코스”보다 자신에게 맞는 제외 기준이 더 중요합니다.</p>`,
    `<h2>비 온 뒤 숲길에서 실제로 달라지는 것</h2>
<p>비가 그친 뒤 숲길은 공기와 색감은 좋아지지만, 난이도는 평소보다 올라갑니다. 특히 낙엽 아래 젖은 흙, 계단 모서리의 얇은 물막, 이끼 낀 돌은 사진으로는 잘 보이지 않습니다. 그래서 후기에서 “걷기 좋다”는 말만 보면 안 되고, 하산로가 어떤 재질인지까지 봐야 합니다.</p>
<p>가장 안정적인 선택은 임도와 숲길이 섞인 낮은 회귀형 코스입니다. 이런 코스는 중간에 날씨가 다시 나빠져도 방향을 바꾸기 쉽고, 신발이 젖었을 때도 속도를 조절하기 좋습니다. 반대로 계곡을 여러 번 건너는 코스는 전날 비가 많았다면 후보에서 빼는 것이 맞습니다.</p>
<h2>장비보다 중요한 보행 습관</h2>
<p>젖은 길에서는 비싼 방수화보다 짧은 보폭이 더 중요합니다. 발을 멀리 뻗으면 미끄러질 때 몸을 회복하기 어렵습니다. 스틱을 쓴다면 몸보다 너무 멀리 찍지 말고, 발보다 반걸음 앞에 가볍게 두어 균형을 보조하는 방식이 좋습니다.</p>`,
    `<h2>단풍 산행에서 검색 결과가 놓치는 것</h2>
<p>단풍 글은 대부분 “어디가 예쁜가”를 말하지만, 실제 만족도는 “얼마나 막히지 않고 걷는가”에서 갈립니다. 단풍 절정 주말에는 정상부보다 주차장 진입, 셔틀 대기, 전망대 계단, 하산 식당가가 더 큰 변수입니다. 그러므로 산 자체보다 병목 구간을 먼저 찾아야 합니다.</p>
<p>내장산이나 설악산 같은 대표 단풍지는 절정일에 정면으로 들어가면 사진보다 대기 시간이 길어질 수 있습니다. 이때는 정상 왕복을 버리고 낮은 숲길, 사찰 연결길, 능선 일부만 걷는 전략이 더 낫습니다. 단풍은 꼭 정상에서만 보이는 것이 아니고, 오히려 낮은 계곡과 사찰 주변이 색이 풍부한 경우도 많습니다.</p>
<h2>혼잡을 피하는 구체적 선택</h2>
<p>평일에 갈 수 없다면 절정 3일 전 또는 절정 이후 첫 평일을 노리세요. 색은 조금 덜하거나 살짝 지나도 걷는 흐름이 훨씬 좋습니다. 사진 목적이라면 포토존 두 곳만 정하고 나머지는 걷는 시간을 확보하는 것이 좋습니다.</p>`,
    `<h2>겨울 낮은 산도 방심하면 위험한 이유</h2>
<p>겨울 산행 사고는 높은 산에서만 생기지 않습니다. 낮은 산이라도 북사면 그늘, 얼어붙은 돌계단, 낙엽 아래 결빙이 있으면 하산 난이도는 크게 올라갑니다. 초보에게 낮은 산을 권하는 이유는 쉬워서가 아니라, 문제가 생겼을 때 빨리 내려올 수 있기 때문입니다.</p>
<p>겨울 첫 산행은 왕복 3시간 안쪽, 원점회귀, 들머리 접근이 쉬운 곳이 좋습니다. 정상 도착 목표는 정오 전후로 잡고, 오후 2시 이후에는 능선에 오래 머물지 않는 기준을 세워야 합니다. 해가 짧아지는 계절에는 30분 지연이 체감 위험으로 바뀝니다.</p>
<h2>아이젠보다 먼저 확인할 것</h2>
<p>아이젠은 필요하지만 만능이 아닙니다. 결빙된 바위와 젖은 나무계단에서는 아이젠을 신어도 미끄러질 수 있습니다. 출발 전 기온, 풍속, 전날 강수, 일몰 시간을 확인하고, 하산로가 그늘인지까지 보는 것이 겨울 코스 선택의 핵심입니다.</p>`,
    `<h2>KTX 산행에서 비용보다 중요한 피로 관리</h2>
<p>KTX를 타면 운전 피로는 줄지만, 일정 설계를 잘못하면 산행 피로는 오히려 커질 수 있습니다. 늦은 밤 도착, 짧은 수면, 새벽 택시 이동, 긴 산행, 저녁 열차 복귀가 한 번에 겹치면 운전하지 않아도 몸은 무리합니다. 첫 KTX 명산 여행은 산행 난이도를 낮추고 이동 품질을 높이는 쪽이 좋습니다.</p>
<p>숙소는 역에서 가까운 곳보다 다음날 들머리 이동이 쉬운 곳이 더 유리할 수 있습니다. 아침 버스가 없거나 택시 호출이 어려운 지역이라면 역세권 숙소가 오히려 불편합니다. 전날 숙소에 도착한 뒤 다음날 이동 수단을 한 번 더 확인해두면 일정이 훨씬 안정됩니다.</p>
<h2>추천 일정 구조</h2>
<p>첫날은 이동과 숙박, 둘째 날은 4~5시간 산행과 여유 있는 복귀로 구성하세요. 하산 후 열차까지 최소 2시간을 비워두면 식사, 환복, 역 이동을 무리 없이 처리할 수 있습니다.</p>`,
    `<h2>계곡 산행의 핵심 위험은 내 위치 밖에서 시작됩니다</h2>
<p>여름 계곡 산행이 까다로운 이유는 내가 있는 곳의 날씨만으로 판단할 수 없기 때문입니다. 상류에 소나기가 내리면 하류 계곡 수위가 빠르게 변할 수 있습니다. 그래서 계곡 산행에서는 강수확률보다 시간당 강수량, 상류 예보, 계곡 횡단 지점을 먼저 봐야 합니다.</p>
<p>좋은 계곡 코스는 물을 가까이에서 보되 직접 건너는 구간이 적은 길입니다. 데크나 안전 난간이 있고, 중간에 임도 또는 능선으로 빠지는 길이 있으면 더 좋습니다. 반대로 협곡형 지형이나 양쪽이 막힌 길은 비 예보가 조금만 있어도 초보에게 맞지 않습니다.</p>
<h2>현장에서 바로 철수해야 하는 신호</h2>
<p>물이 갑자기 탁해지거나, 낙엽과 나뭇가지가 빠르게 떠내려오거나, 물소리가 갑자기 커지면 즉시 높은 길로 이동해야 합니다. 계곡에서 기다리며 상황을 지켜보는 것은 좋은 판단이 아닙니다.</p>`,
    `<h2>일출 산행의 성공 기준은 사진이 아닙니다</h2>
<p>첫 일출 산행은 멋진 사진보다 안전한 왕복이 우선입니다. 어둠 속에서는 같은 길도 낮보다 길게 느껴지고, 갈림길 하나를 놓치면 불필요한 체력 소모가 생깁니다. 그래서 일출 명소보다 길이 단순하고 하산이 쉬운 전망대를 목표로 잡는 것이 좋습니다.</p>
<p>일출 시각이 오전 5시 20분이라면 정상 도착 목표는 최소 20분 전입니다. 표준 소요시간이 70분인 코스라도 초보는 어둠 속 이동을 고려해 90분 이상으로 계산하세요. 정상에서 오래 머물면 체온이 떨어지므로 방풍 재킷과 따뜻한 음료도 필요합니다.</p>
<h2>혼자 갈 수 있는 조건</h2>
<p>낮에 이미 걸어본 길, 갈림길이 적은 길, 휴대전화가 터지는 길이라면 단독 일출 산행도 가능성이 있습니다. 처음 가는 산이라면 동행을 구하거나 낮 산행으로 먼저 답사하는 편이 맞습니다.</p>`,
    `<h2>암릉 공포는 체력 문제가 아닙니다</h2>
<p>암릉이 무서운 사람은 운동을 못해서가 아니라 노출감에 민감한 경우가 많습니다. 이때 필요한 것은 더 강한 마음가짐이 아니라 우회로가 있는 코스 선택입니다. 조망은 바위 정상에서만 얻는 것이 아니며, 데크 전망대나 능선 중간 조망점으로도 충분히 좋은 산행이 됩니다.</p>
<p>후기를 볼 때 “스릴”, “짜릿”, “손을 써야 한다”는 표현이 많으면 초보에게 부담스러운 구간일 가능성이 큽니다. 반대로 계단, 난간, 우회로, 중간 전망대가 언급되면 난이도 조절이 가능합니다.</p>
<h2>현장 판단 기준</h2>
<p>바위 앞에서 몸이 굳으면 더 진행하지 말고 넓은 곳으로 이동하세요. 뒤사람을 의식해 서두르는 순간 사고 가능성이 커집니다. 정상 포기는 실패가 아니라 다음 산행을 가능하게 하는 선택입니다.</p>`,
    `<h2>봄꽃 산행은 걷는 흐름이 무너지기 쉽습니다</h2>
<p>봄꽃 산행은 풍경이 아름답지만, 포토존마다 사람이 멈추기 때문에 생각보다 피로가 큽니다. 개화율이 높은 날일수록 사람 밀도도 올라갑니다. 그래서 꽃 명소를 고를 때는 개화 정보와 함께 군락지 병목, 하산 교통, 화장실 위치를 같이 봐야 합니다.</p>
<p>진달래는 낮은 능선과 초봄 산행에 잘 맞고, 철쭉은 늦봄 고도가 있는 군락지에서 강한 색감을 보여줍니다. 두 꽃은 같은 봄꽃이지만 산행 방식이 다릅니다. 진달래는 걷는 흐름, 철쭉은 군락지 체류와 혼잡 관리가 중요합니다.</p>
<h2>사진보다 좋은 계획</h2>
<p>사진 지점을 두 곳만 정하고 나머지는 걷는 시간으로 남겨두세요. 꽃 절정 주말에는 정상보다 군락지 회귀 코스가 더 만족스러운 경우가 많습니다.</p>`,
    `<h2>10km 종주는 숫자보다 피로의 모양을 봐야 합니다</h2>
<p>능선 종주 초보가 가장 많이 하는 착각은 거리만 보는 것입니다. 산의 10km는 평지의 10km와 다릅니다. 작은 봉우리를 여러 번 넘으면 누적고도가 빠르게 쌓이고, 마지막 하산에서 무릎 피로가 한꺼번에 옵니다. 그래서 거리보다 고도 그래프와 탈출로가 먼저입니다.</p>
<p>첫 종주는 8~10km, 누적고도 700m 이하, 중간 탈출로 2곳 이상이 적당합니다. 능선 위 식수 보급이 없으면 물을 넉넉히 잡고, 행동식은 한 번에 먹는 도시락보다 60~90분마다 먹는 작은 간식이 좋습니다.</p>
<h2>완주보다 중요한 기록</h2>
<p>첫 종주에서는 완주 여부보다 구간별 피로를 기록하세요. 어느 지점에서 속도가 떨어졌는지, 하산 무릎 통증이 언제 시작됐는지 적어두면 다음 종주 계획이 훨씬 정확해집니다.</p>`,
  ][index]
